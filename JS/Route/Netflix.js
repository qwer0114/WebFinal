...
const getDrama = async () => {
  const api = await fetch(
    `${API_URL}discover/tv/?api_key=${API_Key}&page=1&sort_by=popularity.desc&watch_region=KR&with_watch_providers=8&language=ko-KR`
  );
  const json = await api.json();
...
const getKDrama = async () => {
  const api = await fetch(
    `${API_URL}discover/tv/?api_key=${API_Key}&page=1&sort_by=popularity.desc&watch_region=KR&with_original_language=ko&with_watch_providers=8&language=ko-KR`
  );
  const json = await api.json();
